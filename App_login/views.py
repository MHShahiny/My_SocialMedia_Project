...
from django.urls import reverse  # Import reverse to fix the redirect issue
from App_login.forms import CreateNewUser, EditProfile
from django.contrib.auth import login, logout, authenticate  # Corrected import
...
def sign_up(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user_profile.save()
            return HttpResponseRedirect(reverse('App_login:login'))  # Fixed reverse function usage
    
    return render(request, 'App_login/signup.html', context={'title': 'Sign up - Naither', 'form': form})
...
def login_page(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return HttpResponseRedirect(reverse('App_Posts:home'))  # Redirect after successful login
...
def profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            post.author = request.user  # Ensure the author is the logged-in user
            post.save()
